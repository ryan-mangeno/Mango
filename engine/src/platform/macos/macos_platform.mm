...
b8 Platform::startup(const AppConfig &config) {
  ...
  m_internal_state = static_cast<InternalState *>(
      mg_allocate(sizeof(InternalState), MEMORY_TAG_APPLICATION));
  ...
  u32 style_mask = NSWindowStyleMaskTitled | NSWindowStyleMaskClosable |
...
void Platform::shutdown() {
  ...
  if (state->window) {
    [state->window
        setDelegate:nil];  // Prevent delegate callbacks during teardown
...
f64 Platform::get_absolute_time() noexcept {
  ...
  f64 now_time = static_cast<f64>(now.tv_sec) +
                 static_cast<f64>(now.tv_nsec) * static_cast<f64>(1e-9);
