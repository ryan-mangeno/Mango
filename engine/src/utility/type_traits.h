...
// declval
template <typename T> remove_reference_t<T> &&declval() noexcept;
...
template <typename T> struct is_trivial {
  static constexpr b8 value = __is_trivial(T);
};
...
template <typename T>
struct is_copyable<T, decltype(void(T(declval<T>()), // copy construct
                                    declval<T &>() = declval<T>() // copy assign
                                    ))> : true_type {};

template <typename T> static constexpr b8 is_copyable_v = is_copyable<T>::value;

// allows overriding of new and delete ops so I can use my own with placement
// new
