...
// destructor helper if objects were allocated with placement new, ~T will not
// be called for you other wise
template <typename T> MGO_API void mg_delete(T *ptr, memory_tag tag) noexcept {
  if (tag == MEMORY_TAG_UNKNOWN) {
    MGO_WARN("Freeing memory with unknown tag. Reclass this allocation");
  }
