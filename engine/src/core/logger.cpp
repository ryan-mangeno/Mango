...
void Logger::log_message(log_level level, const char *message, ...) {
  ...
  b8 is_err = (level == log_level::LOG_LEVEL_FATAL ||
  ...
  // NOTE: Using __builtin_va_list for compatibility across different compilers
  // for
  __builtin_va_list args;
  va_start(args, message);
  vsnprintf(message_buffer, sizeof(message_buffer), message, args);
  va_end(args);
  snprintf(final_buffer, sizeof(final_buffer), "%s%s", level_str,
           message_buffer);
