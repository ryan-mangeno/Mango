...
public:
    ...
    mg_string(const char* cstr) {
        if (!cstr) { _sso_init_empty(); return; }
        _assign_cstr(cstr, (u64)strlen(cstr));
    }

    mg_string(const char* data, u64 len) {
        _assign_cstr(data, len);
    }

    mg_string(const mg_string& other) {
        _copy_from(other);
    }

    mg_string(mg_string&& other) noexcept {
        _move_from(other);
    }
    ...
    mg_string& operator=(const char* cstr) {
        _free_heap();
        if (!cstr) { _sso_init_empty(); return *this; }
        _assign_cstr(cstr, (u64)strlen(cstr));
        return *this;
    }
    ...
    const char* c_str() const {
        return _is_heap() ? _heap.ptr : _sso.buf;
    }
    ...
    u64 length() const {
        return _is_heap() ? _heap.len : (u64)_sso.len;
    }

    u64 capacity() const {
        return _is_heap() ? _heap.cap : SSO_CAP;
    }
    ...
    mg_string& operator+=(char c)                  { push_back(c); return *this; }
    ...
    u64 find(const char* needle, u64 from = 0) const {
        if (!needle || !needle[0]) return NPOS;
        u64 nlen = (u64)strlen(needle);
        u64 len  = length();
        if (nlen > len) return NPOS;
        const char* s = c_str();
...
private:
    ...
    struct sso_rep {
        char buf[SSO_CAP+1]; // buf[15] is the null terminator slot; buf[0..len-1] is data
        u8   len;     // 0..15
        u8   _pad[14];
        u8   flag;    // always 0
    };
