...
/*
the users actualy doesnt need to know about the implementations of keyboard,
mouse, input state slightly more c style here, and focuses on internal linktage.
I am only exposing endpoints to poll input state, which there is really only one
of however, there can be different input systems, keyboard vs gamepad, vs
controller, etc, but down the line I would prefer a dependency injection
approach here in order to avoid alot of polymorphic calls with vtable as input
will likely be polled alot per game update pass numerous times in the
engine/game
*/
...
void input_update(f64 delta_time) noexcept {
  ...
  mg_copy_memory(&s_state.keyboard_previous, &s_state.keyboard_current,
                 sizeof(KeyboardState));
  mg_copy_memory(&s_state.mouse_previous, &s_state.mouse_current,
                 sizeof(MouseState));
}
...
void input_process_key(keys key, b8 pressed) noexcept {
  ...
  // we only change state our button state and logged keyboard state are
  // different
  if (is_down != pressed) {
    if (pressed) {
