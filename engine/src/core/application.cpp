...
b8 Application::create_app(game *game_inst) {
  ...
  if (!input_initialize()) {
    MGO_FATAL(
        "Input system failed to initialize, application can't continue ...");
    return FALSE;
  }

  if (!event_initialize()) {
    MGO_FATAL(
        "Event system failed to initialize, application can't continue ...");
    return FALSE;
  }
  ...
  // s_app_state.game_inst->on_resize(s_app_state.width, s_app_state.height);
  // TODO:
...
void Application::run() {
  ...
  u8 frame_count = 0;
  f64 target_fps =
      1.0 / 60.0; // NOTE: should be adjustible but hardcoding for now
...
b8 Application::_on_event(u16 code, void *sender, void *listener_inst,
                          EventContext ctx) {
...
b8 Application::_on_key(u16 code, void *sender, void *listener_inst,
                        EventContext ctx) {
