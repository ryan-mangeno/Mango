...
template <typename T> class MGO_API darray {
public:
  darray() noexcept : _elements(nullptr), _capacity(0), _length(0) {}

  darray(u64 length, u64 capacity) noexcept
      : _capacity(capacity), _length(length) {
    _elements =
        static_cast<T *>(mg_allocate(_capacity * sizeof(T), MEMORY_TAG_DARRAY));
  ...
  ~darray() {
    if (_elements) {
      if constexpr (!is_trivial_v<T>) {
        for (u64 i = 0; i < _length; ++i) {
          _elements[i]
              .~T(); // we only destroy the object, the block is freed below
        }
      }
  ...
  template <typename... Args> inline void emplace_back(Args &&...args) {
    ...
    mg_construct_at(&_elements[_length], MEMORY_TAG_DARRAY,
                    forward<Args>(args)...);
    _length++;
  }
  ...
  void insert(u64 index, const T &element) {
    ...
    if constexpr (is_trivial_v<T>) {
      ...
      if (elements_to_move > 0) {
        mg_copy_memory(&_elements[index + 1], &_elements[index],
                       elements_to_move * sizeof(T));
      }
  ...
  void erase(u64 index) {
    ...
    if constexpr (is_trivial_v<T>) {
      ...
      if (elements_to_move > 0) {
        // shift mem left by 1
        mg_copy_memory(&_elements[index], &_elements[index + 1],
                       elements_to_move * sizeof(T));
      }
    } else {
  ...
  void resize(u64 new_capacity) {
    ...
    T *new_elements = static_cast<T *>(
        mg_allocate(new_capacity * sizeof(T), MEMORY_TAG_DARRAY));
    ...
    if (_elements) {
      if constexpr (is_trivial_v<T>) {
        ...
        for (u64 i = 0; i < _length; ++i) {
          mg_construct_at(&new_elements[i], MEMORY_TAG_DARRAY,
                          move(_elements[i]));
          _elements[i].~T();
        }
      }
  ...
  inline const T &operator[](u64 index) noexcept { return _elements[index]; }
...
private:
  inline void _grow() { resize(_capacity == 0 ? 4 : _capacity * 2); }
